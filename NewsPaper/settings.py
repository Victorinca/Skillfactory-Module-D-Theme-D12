...
ADMINS = [('admin', 'test@atlant-opt.com')
         # список всех админов в формате ('имя', 'их почта')
]

MANAGERS = [('admin', 'test@atlant-opt.com')
...
LOGGING = {
     ...
     'formatters': {
         'simple': {
             # 1) время; уровень сообщения; сообщение
             'format': '%(asctime)s %(levelname)s %(message)s',
         },
         'simple_path': {
             # 2) время; уровень сообщения; сообщение; путь к источнику события (аргумент pathname)
             'format': '%(asctime)s %(levelname)s %(message)s %(pathname)s',
         },
         'simple_path_error': {
             # 3) время; уровень сообщения; сообщение; путь к источнику события (аргумент pathname); стэк ошибки (аргумент exc_info)
             'format': '%(asctime)s %(levelname)s %(message)s %(pathname)s %(exc_info)s',
         },
         'custom_path_': {
             # 4) время; уровень логирования; сообщение; путь к источнику сообщения
             'format': '{asctime} {levelno} {message} {pathname}',
             'style': '{',
         },
         'custom_path_error_': {
             # 5) время; уровень логирования; сообщение; путь к источнику сообщения; стэк ошибки
             'format': '{asctime} {levelno} {message} {pathname} {exc_info}',
             'style': '{',
         },
         'custom_module_': {
             # 6) время; уровень логирования; модуль, в котором возникло сообщение (аргумент module); сообщение
             'format': '{asctime} {levelno} {module} {message}',
             'style': '{',
         },
     },
# на сообщения будет накладываться фильтр
# или (DEBUG = True),
# или DEBUG = False
# и они будут выводиться с определенным форматом.
    'filters': {
        'require_debug_true': {
    ...
    'handlers': {
        ...
        'general_log': {
             ...
             'formatter': 'custom_module_',
        },
        'errors_log': {
             'level': 'ERROR',
             'class': 'logging.handlers.RotatingFileHandler',
             'filename': f"{BASE_DIR}/logs/errors.log",
             'formatter': 'custom_path_error_',
        },
        'security_log': {
             'level': 'DEBUG',
             'class': 'logging.handlers.RotatingFileHandler',
